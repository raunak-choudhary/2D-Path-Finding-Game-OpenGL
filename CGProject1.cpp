...
void frontscreen(void)
{
	...
	glLoadIdentity();
    glColor3f(1,0.3,0);
    drawstring(10,170,"SHRI MADHWA VADIRAJA INSTITUTE OF TECHNOLOGY AND MANAGEMENT",GLUT_BITMAP_HELVETICA_18);
    glColor3f(0.7,1,0);
    drawstring(25,160,"DEPARTMENT OF COMPUTER SCIENCE AND ENGINEERING",GLUT_BITMAP_HELVETICA_18);
    glColor3f(0.8,0.4,1);
	output(55,130,"A Mini Project On:-");
    glColor3f(1,0.8,0.1);
    drawstring(46,120.0,"2D PATH FINDING GAME",GLUT_BITMAP_TIMES_ROMAN_24);
   // glColor3f(1,0,0);
   // drawstring(40,60,0.0,"R Trees",GLUT_BITMAP_HELVETICA_18);
    glColor3f(1,0.5,0);
    drawstring(-70,50,"BY:",GLUT_BITMAP_HELVETICA_18);
    glColor3f(0.7,0,0.7);
    drawstring(-70,40,"RAUNAK CHOUDHARY        (4MW17CS064)",GLUT_BITMAP_HELVETICA_18);
    glColor3f(0,0.7,0.7);
    drawstring(-70,34,"SHRIHARI KALLAPUR           (4MW17CS080)",GLUT_BITMAP_HELVETICA_18);
    glColor3f(1,0,0.7);
    drawstring(145,50,"Under the Guidence of :",GLUT_BITMAP_HELVETICA_18);
    glColor3f(0.5,1,0.8);
    drawstring(145,40,"Ms. Deepthi G. Pai (Assistant Professor), Dept. of CSE",GLUT_BITMAP_HELVETICA_18);
    //drawstring(63,34,"GUIDE NAME SECOND",GLUT_BITMAP_HELVETICA_18);
    glColor3f(1,0.2,1);
    drawstring(55,15,"PRESS ENTER TO START",GLUT_BITMAP_HELVETICA_18);
    //glutSwapBuffers();
    glFlush();
}
...
void levelscreen()
{
	...
	output(25,140,"CHOOSE A DIFFICULTY LEVEL");
	glColor3f(1.0,0.0,0.0);
	output(50,100,"1.EASY");
	glColor3f(1.0,1.0,0.0);
	output(50,80,"2.MEDIUM");
	glColor3f(0.0,1.0,0.0);
	output(50,60,"3.HARD");
	glFlush();
}
...
void instructions()
{
	glClear(GL_COLOR_BUFFER_BIT);
	glColor3f(1.0,1.0,0.0);
        output(45,170,"INSTRUCTIONS:");
	glBegin(GL_LINES);
	glVertex2f(44,168);
	glVertex2f(88,168);
	glEnd();
	glColor3f(1,0,1);
	output(-20,160,"* TO MOVE THE POINT IN UPWARD DIRECTION: USE 'UP' KEY");
	glColor3f(0.7,1,0);
	output(-20,140,"* TO MOVE THE POINT IN DOWNWARD DIRECTION: USE 'DOWN' KEY");
	glColor3f(1,0,1);
	output(-20,120,"* TO MOVE THE POINT IN LEFT DIRECTION: USE 'LEFT' KEY");
	glColor3f(0.7,1,0);
	output(-20,100,"* TO MOVE THE POINT IN RIGHT DIRECTION: USE 'RIGHT' KEY");
	glColor3f(1,0,1);
    output(-20,80,"* FIND THE WAY TO EXIT MAZE BEFORE COUNTDOWN FINISHES. HURRY UP!!");
    glColor3f(0.7,1,0);
    output(-20,60,"* RED COLOURED POINT INDICATE THE POINT FROM WHERE YOU HAVE TO START");
    glColor3f(1,0,1);
    output(-20,40,"* GREEN COLOURED POINT INDICATE THE POINT WHERE YOU HAVE TO REACH");
    glColor3f(0.7,1,0);
...
void idle()
{
	if(df==1)
  	...
  	if(df==8)
  	{
		...
		else if((count<30) && ((px>=0 && px<=4) && (py>=162 && py<=168)))
		{
			df=5;
		}
  	}
...
void display()
{
	...
	if(df==10)
		...
		if(df==1)
			...
			output(-21,163,"<----");
			glColor3f(0.0,0.0,1.0);
			drawstring(185,160,"TIME REMAINING : ",GLUT_BITMAP_TIMES_ROMAN_24);
			sprintf(t,"%d",90-count);
        		output(240,160,t);
			if(90-count<30)
			{
				glColor3f(1,1,0);
...
void keyboard(unsigned char key,int x,int y)
{
	...
	if((key=='0' || key=='1'||key=='2'||key=='3')&& (df==4||df==1||df==7||df==8))
  	{
   		px=0.0;
   		py=175.0;
  	}
...
int main(int argc,char **argv)
{
	glutInit(&argc,argv);
  	...
  	glutCreateWindow("2D Path Finding Game");
